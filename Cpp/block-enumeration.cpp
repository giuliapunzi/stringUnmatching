...
constexpr auto Q = 32;
constexpr auto B = 20; // block size 
...
void build_block_functions(uint64_t* g, int* offset){
    // srand(SEED);
    // srand(time(NULL));
    ...
    while( !all_covered ){
        ...
        for(int i=0; i<N_hash_fctns; i++){
            ...
            int pos[target_size];
            // pos[0] = rand() % B;
            std::uniform_int_distribution<unsigned> distrib(0, B-1); // B-1 included
            pos[0] = distrib(gen);
            
            int filled = 1;
            bool new_position = true;
            while(filled < target_size){
                // int candidate_pos = rand() % B;
                std::uniform_int_distribution<unsigned> distrib(0, B-1);
                int candidate_pos = distrib(gen);
            ...
            for(int j=0; j<target_size; j++){
                pos[j] = (pos[j] + offset[i]) % Q; // for cyclic rotations
            }
...
int main()
{
    ...
    int f = Q/N_hash_fctns;
    int offset[N_hash_fctns]; // offsets equal to multiples of Q/N_hash_fctns
    
    for(int i = 0; i < N_hash_fctns; i++)
        offset[i] = i*f;

    clock_t begin = clock();
    build_block_functions(g, offset);
    clock_t end = clock();
