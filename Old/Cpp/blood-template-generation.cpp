...
constexpr auto UNIVERSE_SIZE = 4194304; // 4^12 = 16777216;  // 4^14 = 268435456
...
void compute_templates(const uint64_t *g){
    ...
    outputfile.open("../exp_results/Blood/" +to_string(N_hash_fctns) + "Funct" + to_string(target_size) + "TargetSeed" + to_string(SEED), ios::app);
    binaryout.open("../exp_results/Blood/" +to_string(N_hash_fctns) + "FunctBinary" + to_string(target_size) +"TargetSeed"  + to_string(SEED), ios::binary | ios::app);
